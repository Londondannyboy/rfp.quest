  ...
  if (!opportunities || opportunities.length === 0) {
    return (
      <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-100">
        ...
        <p className="text-gray-500 text-sm">No quick wins available right now</p>
      </div>
    );
  }
        ...
        {opportunities.slice(0, 4).map((opp, index) => (
          ...
          >
            <Link
              ...
              className="block bg-white rounded-lg p-4 hover:shadow-md transition-all group border border-transparent hover:border-green-200"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <CheckCircleIcon className="w-4 h-4 text-green-500 shrink-0" />
                    <h4 className="font-medium text-gray-900 text-sm truncate group-hover:text-green-600 transition-colors">
                      {opp.title}
                    </h4>
                  </div>
